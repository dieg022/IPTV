...
<div class="row">
    <div class="col-md-12 col-sm-12 col-xs-12">
        <div class="x_panel">
            ...
            <div class="x_content">
                <br />
                    <form id="main" name="main" class="form-horizontal"  action="{{ URL::to('paquetecanal/'.$paquetecanal->id)}}" method="post" novalidate>

                    <div class="form-group">
                        <label class="col-sm-2 control-label" for="nombrepaquete">Nombre</label>
                        <div class="col-sm-5">
                            <input id="nombrepaquete" class="form-control" type="text" placeholder="Nombre Paquete" name="nombrepaquete" value="{{ old('nombrepaquete',$paquetecanal->nombrepaquete) }}">
                        </div>
                                    ...
                                    <td>
                                        <select id="pais-<?php echo $i;?>" class="form-control" name="pais<?php echo $i;?>" onChange="cambiador(this)">
                                            ...
                                            @foreach($paises as $pais)
                                                ...
                                                @if ( isset($vectorOrden[$i]) && $vectorOrden[$i]['pais'] == $pais->pais)
                                    ...
                                    <td align="center">
                                        <select id="canalpais-<?php echo $i;?>" class="form-control" name="canal;<?php echo $i;?>" onChange="cambiador(this)">
                                            ...
                                            @foreach($canales as $canal)
                                                ...
                                                @if ( isset($vectorOrden[$i]) && $vectorOrden[$i]['canal_id'] == $canal->id)
                                                    <option value="{{$canal->pais}} {{$canal->canal}}-{{$canal->id}}" selected >{{$canal->acronimopais}} {{$canal->canal}}</option>
                                                @else
                    ...
                    <div class="clearfix">&nbsp;</div>
                        ...
                        <div class="form-group">
                            <div class="col-sm-offset-2 col-md-2"
                                 data-form="#borrarclienteformulario" data-title="Eliminar Paquete de Canales" data-message="¿Desea eliminar el este paquete de canales?<br><BR> <B>IMPORTANTE</B>:Si estuviese seleccionado
por un perfil de instalación, éste también será eliminado">
                                <a class = "formConfirm btn btn-danger" href="" class="btn btn-danger">Eliminar</a>
                            </div>
                            <div class="col-md-2"> <a href="{{ url('paquetecanal')}}" class="btn btn-default">Cancelar</a></div>
                            <div class="col-md-4"><button style="align:right;" type="submit" class="btn btn-primary">Actualizar</button> </div>
                        </div>
                ...
                <form id="borrarclienteformulario" name=borrarclienteformulario" action="{{ URL::to('paquetecanal/'.$paquetecanal->id)}}" method="post" novalidate>
...
<script>
    ...
    $(document).ready(function(){
        ...
        for (var j = 1; j <{{$numeroCanales}}; j++) {
            ...
            //al principio todos los selects deshabilitados

            selectPais = document.getElementById("pais-"+j);

            nbrCompuesto="canal"+"pais-"+j;
            selectobject = document.getElementById(nbrCompuesto);

            if(selectPais.value == "Seleccione")
            ...
            //alert("loigitud2="+longitud2);

            //cada vez que hay un cambio habilito todos
            ...
            for (var i=longitud2-1; i>=0; i--){
                ...
                valor = document.getElementById("pais-"+j);

               // alert(valor.value);
                //por aki me kedo tengo k sacar en valor.value el nombre del pais de... sus muerto XDDd no lo se mu bien
    ...
    /*
    TENDRIA QUE HACER CON OTRO SCRIPT UNA PRIMERA PASADA PARA PONER BIEN LOS CHECKOSX
    TB ME KEDA CAMBIAR EL CODIGO DEL UPDATE Y A CORRER!!!
     */
    function cambiador(valor)
    {
        ...
        for (var i=longitud2-1; i>=0; i--){
            ...
            if(!selectobject.options[i].value.includes(valor.value)){
                ...
                selectobject.options[i].style.background="green";
                selectobject.options[i].style.color="white";
                //alert(selectobject.options[i].value + " si l otiene => no lo borro");
            }
        }
